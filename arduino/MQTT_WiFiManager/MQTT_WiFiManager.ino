...
#define mqtt_topico_sub           "smarthome/lampada"    //Tópico para subscrever o comando a ser dado no pino declarado abaixo
#define mqtt_topico_pub_temp      "smarthome/temp"       //Tópico que será informado o valor da temperatura
#define mqtt_topico_pub_umidade   "smarthome/umidade"    //Tópico que será informado o valor da umidade

//Declaração do pino que será utilizado e a memória alocada para armazenar o status deste pino na EEPROM
#define pino                      13                  //Pino que executara a acao dado no topico "smarthome/lampada" e terá seu status informado no tópico "smarthome/lampada"
#define memoria_alocada           4                   //Define o quanto sera alocado na EEPROM (valores entre 4 e 4096 bytes)
#define DHTPIN                    12                  //Pino que recebe o valor da temperatura e terá seu status informado no tópico "smarthome/temp" 
...
void enviaDHT(){
  ...
  else 
  {
    ...
    sprintf(MsgUmidadeMQTT,"%2.2f",umidade);
    client.publish(mqtt_topico_pub_umidade, MsgUmidadeMQTT);
    sprintf(MsgTemperaturaMQTT,"%2.2f",temperatura);
    client.publish(mqtt_topico_pub_temp, MsgTemperaturaMQTT);
  }
}
