...
import br.edu.ifrn.smart.utils.extensions.flags
...
object ConnectionManager {
    ...
    fun connect(appContext: Context) {
        ...
        client?.setCallback(object : MqttCallback {
            ...
            override fun connectionLost(cause: Throwable?) {
                if (BuildConfig.DEBUG)
                    Log.e("ConnectionManager", "Conecxao perdida", cause)
    ...
    private fun showConnectionLostDialog() {
        val alert = context?.alert("Conecxao perdida", "Deseja reconectar ao broker?") {
            positiveButton("Sim") { connect(context!!) }
            negativeButton("Nao") {
        ...
        try {
            ...
            e.printStackTrace()
            context?.longToast("Conecxao perdida!")
        }
    }
